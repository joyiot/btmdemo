...
define('ODIN_JOYGUESS_PREFIX','ppk:JOY/guessgame/');
define('PPK_JOYPUB_FLAG','JPUB');
define('ODIN_JOYPUB_PREFIX','ppk:JOY/pub/');
define('PPK_JOYIOT_FLAG','JIOT');
define('ODIN_JOYIOT_PREFIX','ppk:527064.583/iot/');
...
if(strcasecmp($gStrBtmNetworkId,'mainnet')==0){
  ...
  define('BTM_NODE_API_TOKEN','');  //API访问密钥
...
function getNextAccountInfo(){
  ...
  $nextAccountSN=1+intval(file_get_contents(PLUS_SITE_HTDOCS_PATH.'LastAccountSN.txt'));
  if($nextAccountSN>=count($gArrayNodeAccounts))
     $nextAccountSN=0;
  file_put_contents(PLUS_SITE_HTDOCS_PATH.'LastAccountSN.txt',$nextAccountSN);
